...
class categoryColumn {
  constructor(isEditMode, cols) {
    ...
    this.defaultColDef = field => ({
      ...
      sortable: true,
      ...categoryFilters.chooseAptFilter(field.dataType),
    ...
    this.getFields = fields =>
      (this.allColumns = [
        ...
        fields.map(field => {
          if (field.name === "parentCategoryName") {
            const { cellRendererSelector, ...rest } = this.defaultColDef(field);
            return {
              ...rest,
            ...
            return {
              ...
              cellRendererFramework: params => {
                return (
                  <Link
                    to={{
                      pathname: ROUTE_URLS.NEW_CATEGORY_URL,
                      state: {
                        isEditMode: true,
                        rowData: params.data
                      }
...
const categoryFilters = new FilterOptions({
  collection: "category"
});
